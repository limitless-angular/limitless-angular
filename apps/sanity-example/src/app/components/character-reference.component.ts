...
@Component({
  ...
  template: `
    @if (characterData(); as data) {
      <a
        [href]="'#/pretend-this-works/' + value()?.characterId"
        (click)="$event.preventDefault()"
        class="peer whitespace-nowrap"
      >
...
export class CharacterReferenceComponent extends PortableTextMarkComponent<CharacterReferenceMark> {
  characterData = computed<CharacterDefinition | undefined>(() => {
    const id = this.value()?.characterId ?? '';
