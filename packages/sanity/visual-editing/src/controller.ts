...
import { handleOverlayDrag } from './util/drag-and-drop';
...
export function createOverlayController({
  ...
  function removeEventHandlers(el: ElementNode, handlers: EventHandlers) {
    ...
    el.removeEventListener(
      'contextmenu',
      handlers.contextmenu as EventListener,
      {
        capture: true,
      },
    );
    ...
    const eventHandlers: EventHandlers = {
      ...
      mousemove(event) {
        ...
        if (el) {
          el.addEventListener(
            'mouseenter',
            eventHandlers.mouseenter as EventListener,
          );
          el.addEventListener(
            'mouseleave',
            eventHandlers.mouseleave as EventListener,
          );
        }
      },
      mouseenter() {
        ...
        function addDeferredLeave(el: ElementNode) {
          const deferredLeave = (e: MouseEvent) => {
            ...
            if (!deferredContainer) {
              el.removeEventListener(
                'mouseleave',
                deferredLeave as EventListener,
              );
              leave();
  ...
  function handleMutation(mutations: MutationRecord[]) {
    ...
    for (const mutation of mutations) {
      ...
      const node: Node | null =
        type === 'characterData' ? target.parentElement : target;
  ...
  function activate() {
    ...
    elementSet.forEach((element) => intersectionObserver.observe(element));
    handler({
