...
export function findSanityNodes(
  ...
  function resolveNode(node: ChildNode): ResolvingElement | undefined {
    ...
    if (
      isElementNode(node) &&
      node.dataset?.['sanityEditTarget'] !== undefined
    ) {
      const nodesInTarget = findSanityNodes(node);
      const commonData = findCommonSanityData(
        nodesInTarget
          .map((node) =>
            node.type === 'element' ? node.commonSanity : undefined,
          )
    ...
    // Check element nodes for data attributes, alt tags, etc
      ...
      if (
        node.tagName === 'SCRIPT' ||
        node.tagName === 'SANITY-VISUAL-EDITING'
      ) {
        return;
      }
  ...
  function processNode(
    ...
    let parentGroup: Omit<ResolvedElement, 'commonSanity'> | undefined =
      _parentGroup;

    if (
      isElementNode(node) &&
      node.dataset?.['sanityEditGroup'] !== undefined
    ) {
      parentGroup = {
        type: 'group',
        elements: {
...
export function sanityNodesExistInSameArray(
  ...
  if (
    !isSanityArrayPath(sanityNode1.path) ||
    !isSanityArrayPath(sanityNode2.path)
  )
    return false;

  return (
    getSanityNodeArrayPath(sanityNode1.path) ===
    getSanityNodeArrayPath(sanityNode2.path)
  );
}
...
export function resolveDragAndDropGroup(
  ...
  if (!sanity || !isSanityNode(sanity) || !isSanityArrayPath(sanity.path))
    return null;
  ...
  const group = [...elementSet].reduce<OverlayElement[]>((acc, el) => {
    ...
    const sharedDragGroup =
      targetDragGroup !== null ? targetDragGroup === elDragGroup : true;
