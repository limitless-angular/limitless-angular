...
export function rayRectIntersections(
  line: Ray2D,
  rect: OverlayRect,
): Array<Point2D> | false {
  ...
  for (let i = 0; i < rectLines.length; i++) {
    ...
    if (intersection) {
      ...
      for (let j = 0; j < intersections.length; j++) {
        if (
          intersections[j].x === intersection.x &&
          intersections[j].y === intersection.y
        ) {
          isDuplicate = true;
        }
      }
  ...
  return intersections.sort(
    (a, b) =>
      pointDist(a, { x: line.x1, y: line.y1 }) -
      pointDist(b, { x: line.x1, y: line.y1 }),
  );
}
  ...
  for (const target of targets) {
    ...
    if (intersections) {
      ...
      if (closestIntersection) {
        if (
          pointDist(rayOrigin, firstIntersection) <
          pointDist(rayOrigin, closestIntersection)
        ) {
