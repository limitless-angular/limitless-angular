...
function calcMousePos(e: MouseEvent) {
  ...
  return {
    x: Math.max(
      bodyBounds.x,
      Math.min(e.clientX, bodyBounds.x + bodyBounds.width),
    ),
...
function buildPreviewSkeleton(
  ...
  const children = Array.from(
    element.querySelectorAll(
      ':where(h1, h2, h3, h4, p, a, img, span, button):not(:has(*))',
    ),
  );
...
function calcMinimapTransformValues(
  rects: OverlayRect[],
  groupHeightOverride: number | null,
) {
  ...
  const scaleFactor =
    groupHeight > window.innerHeight ? window.innerHeight / groupHeight : 1;
  const scaledRects = rects.map((r) =>
    scaleRect(r, scaleFactor, { x: window.innerWidth / 2, y: 0 }),
  );
...
function calcGroupBoundsPreview(rects: OverlayRect[]) {
  ...
  const canOffsetY =
    ...
    groupBoundsY.min + groupBoundsY.height <=
      document.body.scrollHeight - offsetDist;
...
async function resetMinimapWrapperTransform(
  ...
  return new Promise((resolve) => {
    ...
    document.documentElement.style.overflow =
      previousRootStyleValues.documentElement.overflow;
    document.documentElement.style.height =
      previousRootStyleValues.documentElement.height;
  });
}
...
export function handleOverlayDrag(opts: HandleOverlayDragOpts): void {
  ...
  const flow = (element.getAttribute('data-sanity-drag-flow') ||
    calcTargetFlow(rects)) as 'horizontal' | 'vertical';

  const dragGroup = element.getAttribute('data-sanity-drag-group');

  const disableMinimap = !!element.getAttribute(
    'data-sanity-drag-minimap-disable',
  );

  const preventInsertDefault = !!element.getAttribute(
    'data-sanity-drag-prevent-default',
  );

  const documentHeightOverride = element.getAttribute(
    'data-unstable_sanity-drag-document-height',
  );
  const groupHeightOverride = element.getAttribute(
    'data-unstable_sanity-drag-group-height',
  );
  ...
  const handleMouseMove = (e: MouseEvent): void => {
    ...
    if (JSON.stringify(insertPosition) !== JSON.stringify(newInsertPosition)) {
      ...
      handler({
        ...
        insertPosition: resolveInsertPosition(
          overlayGroup,
          insertPosition,
          flow,
        ),
